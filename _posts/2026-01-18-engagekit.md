...
Run engagekit with the target name and scope before touching any tools
Fill out `00-Scope.md`: IP range, in/out of scope, rules of engagement, any credentials provided
Work through `notes/01` through `notes/05` as the engagement progresses, keeping commands and findings in the relevant phase file
Drop raw tool output directly into `scans/<tooltype>/` — nmap XML files go in `scans/nmap/`, ffuf output in `scans/web/`, and so on
Use `evidence/capture.sh` to save key command output with timestamps as you go, not after the fact
Track everything sensitive in `loot/loot.md` as you find it
Build findings in `notes/06-Findings.md` throughout the engagement so the CVSS blocks and MITRE mappings are done before you sit down to write the report
...
Source
Full source and documentation: github.com/sireton/engagekit
See also: reconX Recon Automation Script
