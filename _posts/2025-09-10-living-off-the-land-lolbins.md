...
**MITRE ATT&CK mapping:** [T1105 - Ingress Tool Transfer](https://attack.mitre.org/techniques/T1105/), [T1048.003 - Exfiltration Over Unencrypted Non-C2 Protocol](https://attack.mitre.org/techniques/T1048/003/)
...
**Process execution logging with full arguments is non-negotiable here.** Without command-line arguments, you see `python3` ran. With them, you see it spawned a socket and exec'd a shell. The difference in detection value is absolute.

For curl and wget, auditd `exe=` path matching works cleanly because those are real binaries with stable paths. For Python, it does not. On Debian and Ubuntu systems, `/usr/bin/python3` is a symlink and auditd's `-F exe=` filter matches the resolved binary path from `/proc/PID/exe`, which will be the versioned interpreter (`/usr/bin/python3.12`, `/usr/bin/python3.11`, etc.) rather than the symlink. A rule written against `/usr/bin/python3` will not fire. The version-pinned path breaks on interpreter upgrades. The correct approach is to drop the `exe=` filter and capture all `execve` calls, then filter in the SIEM by `EXECVE` records where `argv[0]` contains `python`:

```
-a always,exit -F arch=b64 -S execve -k interpreter_exec
-a always,exit -F arch=b32 -S execve -k interpreter_exec
```

This is broader, but it gives you complete coverage without path fragility. Filter and tune on the SIEM side. For Perl and Bash, the binaries are real files at stable paths and `exe=` matching works as expected:

```
-a always,exit -F arch=b64 -S execve -F exe=/usr/bin/perl -k perl_exec
-a always,exit -F arch=b64 -S execve -F exe=/usr/bin/bash -k bash_exec
```

Note that on distributions where `/bin` is a symlink to `/usr/bin` (Ubuntu 20.04+, Debian 10+), `/bin/bash` and `/usr/bin/bash` resolve to the same inode, so either path in the rule will match. On RHEL and CentOS where they are separate directories, you may need rules for both paths.
